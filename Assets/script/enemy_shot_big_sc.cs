...
public class enemy_shot_big_sc : MonoBehaviour
{
    ...
    void main_phase()
    {
        ...
        if (t_c / 1 % 3 == 0) { enemy_shot_pic.sprite = nomal_cg; }
        if (t_c / 1 % 3 == 1) { enemy_shot_pic.sprite = nomal_cg2; }
        if (t_c / 1 % 3 == 2) { enemy_shot_pic.sprite = nomal_cg3; }
    ...
    void coll_on(Collider2D other)
    {
        ...
        if (other.name.IndexOf("man") >= 0)
        {
            ...
            {
                if (other.GetComponent<man_cs>() && my.muteki == 0)
                {
